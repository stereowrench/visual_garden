defmodule VisualGardenWeb.EventLogLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:transferred_amount]} type="number" label="Transferred amount" step="any" />
  ...
  @impl true
  def update(%{event_log: event_log} = assigns, socket) do
    changeset = Gardens.change_event_log(event_log)
  ...
  def handle_event("validate", %{"event_log" => event_log_params}, socket) do
    changeset =
      ...
      |> Gardens.change_event_log(event_log_params)
  ...
  defp save_event_log(socket, :new, event_log_params) do
    case Gardens.create_event_log(event_log_params) do
      {:ok, event_log} ->
        notify_parent({:saved, event_log})
